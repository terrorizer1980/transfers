...
describe("Withdraw", () => {
  ...
  const createInitialState = async (
    ...
    return {
      ...
      state: {
        initiatorSignature: await alice.signMessage(data),
  ...
  const createTransfer = async (
    ...
    const ret = (
      await withdraw.functions.create(encodedBalance, encodedState)
    )[0];
    return ret;
  };
  ...
  const resolveTransfer = async (
    ...
    const encodedBalance = encodeBalance(balance);
    const ret = (
      await withdraw.functions.resolve(
        encodedBalance,
        encodedState,
        encodedResolver
      )
  ...
  describe("Create", () => {
    ...
    it("should fail if recipient has nonzero balance", async () => {
      ...
      await expect(createTransfer(balance, state)).revertedWith(
        "Withdraw: NONZERO_RECIPIENT_BALANCE"
      );
    });
    ...
    it("should fail if the initiators signature is incorrect", async () => {
      const { balance, state } = await createInitialState(getRandomBytes32(), {
        state: {
          initiatorSignature: await alice.signMessage(getRandomBytes32()),
        },
      });
      await expect(createTransfer(balance, state)).revertedWith(
  ...
  describe("Resolve", () => {
    it("should resolve successfully", async () => {
      const data = getRandomBytes32();
      const { balance, state } = await createInitialState(data);
      const responderSignature = await bob.signMessage(data);
