...
const { EtherSymbol } = ethers_1.constants;
const { commify, parseUnits, formatUnits } = ethers_1.utils;
class Currency {
    constructor(type, amount, daiRate) {
        ...
        this.toWad = (n) => parseUnits(n.toString(), 18);
        this.toRay = (n) => this.toWad(this.toWad(n.toString()));
        this.fromWad = (n) => formatUnits(n.toString(), 18);
        ...
        try {
            this.wad = this.toWad(amount._hex ? ethers_1.BigNumber.from(amount._hex) : amount);
            this.ray = this.toRay(amount._hex ? ethers_1.BigNumber.from(amount._hex) : amount);
        }
        catch (e) {
    ...
    toBN() {
        return ethers_1.BigNumber.from(this._round(this.amount));
    }
    format(_options) {
    ...
    round(decimals) {
        ...
        if (typeof decimals === "number" && decimals < nDecimals) {
            const roundUp = ethers_1.BigNumber.from(`5${"0".repeat(18 - decimals - 1)}`);
