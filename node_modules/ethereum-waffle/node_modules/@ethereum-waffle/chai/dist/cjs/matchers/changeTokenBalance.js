...
async function getBalanceChangeForTransactionCall(transactionCall, token, account) {
    const balanceBefore = await token.balanceOf(await account_1.getAddressOf(account));
    await transactionCall();
    const balanceAfter = await token.balanceOf(await account_1.getAddressOf(account));
